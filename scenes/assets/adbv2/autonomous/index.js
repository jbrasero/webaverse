...
export default () => {
  ...
  let targetSpec = null;
  useActivate(() => {
  //alert('activate button2');
var dbname="";

  function post() {
            //  var x = document.getElementById("mySelect").value;
              var url = "http://localhost:3000/createAutonomousDatabase?compartmentId=ocid1.compartment.oc1..aaaaaaaakqwqnhktr7tdq3mqahejxaotzg4ejyce64yjtsmyqvzc7af4dq6a&cpuCoreCount=2";
            //  var data = { "name": x };
              fetch(url, {
                  method: 'POST', // or 'PUT'
...
post();
    ...
    overlay.style.outline = 'none';
    overlay.style.zIndex = '999';
    overlay.innerHTML = "Your autonomous database '"+dbname+"' has been succesfully provisioned.";
    //alert('name:'+dbnameg);
    document.body.appendChild( overlay );
...
*/
  ...
  let physicsIds = [];
  (async () => {
    const u = `${baseUrl}data_center_low-polyv2.glb`;
    let o = await new Promise((accept, reject) => {
      const {gltfLoader} = useLoaders();
      gltfLoader.load(u, accept, function onprogress() {}, reject);
  //    gltfLoader.rotation.x = 100;
    });
